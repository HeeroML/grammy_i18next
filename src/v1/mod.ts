import type { Context, HearsContext, MiddlewareObj } from "grammy";
...
/**
 * The i18next plugin for grammY 1.x.
 *
 ...
 * bot.use(i18n);
 * bot.command("start", (ctx) => ctx.reply(ctx.t("greeting")));
 * ```
 *
...
> extends I18nextCore<C, Ns> implements MiddlewareObj<I18nextFlavor<C, Ns>> {
    ...
    override hears(
        ...
        return <FC extends I18nextFlavor<C, Ns>>(
            ctx: FC,
        ): ctx is HearsContext<FC> => predicate(ctx);
    }
}
