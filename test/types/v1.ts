/**
 * Type-level tests for the grammY 1.x bindings. This file is only type
...
bot.filter(plugin.hears("button.report"), (ctx) => {
    expectType<TFunction>(ctx.t);
    expectType<MySession>(ctx.session);
    // grammY 1.x narrows `ctx.match` for `hears`-style predicates.
    expectType<string | RegExpMatchArray>(ctx.match);
});
